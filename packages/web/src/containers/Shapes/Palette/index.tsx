import React, { type RefObject, useEffect, useRef, useState } from "react";
import Color from "colorjs.io";
import styles from "./styles.module.scss";

// https://stackoverflow.com/questions/13586999/color-difference-similarity-between-two-values-with-js
// https://github.com/antimatter15/rgb-lab
// function deltaE(rgbA:[number,number,number], rgbB:[number,number,number]) {
//   let labA = rgb2lab(rgbA);
//   let labB = rgb2lab(rgbB);
//   let deltaL = labA[0] - labB[0];
//   let deltaA = labA[1] - labB[1];
//   let deltaB = labA[2] - labB[2];
//   let c1 = Math.sqrt(labA[1] * labA[1] + labA[2] * labA[2]);
//   let c2 = Math.sqrt(labB[1] * labB[1] + labB[2] * labB[2]);
//   let deltaC = c1 - c2;
//   let deltaH = deltaA * deltaA + deltaB * deltaB - deltaC * deltaC;
//   deltaH = deltaH < 0 ? 0 : Math.sqrt(deltaH);
//   let sc = 1.0 + 0.045 * c1;
//   let sh = 1.0 + 0.015 * c1;
//   let deltaLKlsl = deltaL / (1.0);
//   let deltaCkcsc = deltaC / (sc);
//   let deltaHkhsh = deltaH / (sh);
//   let i = deltaLKlsl * deltaLKlsl + deltaCkcsc * deltaCkcsc + deltaHkhsh * deltaHkhsh;
//   return i < 0 ? 0 : Math.sqrt(i);
// }

// function rgb2lab(rgb: [number,number,number]){
//   let r = rgb[0] / 255, g = rgb[1] / 255, b = rgb[2] / 255, x, y, z;
//   r = (r > 0.04045) ? Math.pow((r + 0.055) / 1.055, 2.4) : r / 12.92;
//   g = (g > 0.04045) ? Math.pow((g + 0.055) / 1.055, 2.4) : g / 12.92;
//   b = (b > 0.04045) ? Math.pow((b + 0.055) / 1.055, 2.4) : b / 12.92;
//   x = (r * 0.4124 + g * 0.3576 + b * 0.1805) / 0.95047;
//   y = (r * 0.2126 + g * 0.7152 + b * 0.0722) / 1.00000;
//   z = (r * 0.0193 + g * 0.1192 + b * 0.9505) / 1.08883;
//   x = (x > 0.008856) ? Math.pow(x, 1/3) : (7.787 * x) + 16/116;
//   y = (y > 0.008856) ? Math.pow(y, 1/3) : (7.787 * y) + 16/116;
//   z = (z > 0.008856) ? Math.pow(z, 1/3) : (7.787 * z) + 16/116;
//   return [(116 * y) - 16, 500 * (x - y), 200 * (y - z)]
// }

function hex(n: number) {
  const s = n.toString(16);
  return s.length < 2 ? `0${s}` : s;
}

function getPalette(imageData: ImageData) {
  const { data } = imageData;
  const rgbValues = [];
  for (let i = 0; i < data.length; i += 4) {
    const rgb = {
      r: data[i],
      g: data[i + 1],
      b: data[i + 2],
    };
    rgbValues.push(rgb);
  }
  const rgb24Array = rgbValues.map(
    ({ r, g, b }) => `#${hex(r)}${hex(g)}${hex(b)}`
  );
  const counters = rgb24Array.reduce(
    (result, color) =>
      Object.assign(result, {
        [color]: (result[color] || 0) + 1,
      }),
    {} as Record<string, number>
  );
  const sorted = Object.entries(counters).sort((a, b) => b[1] - a[1]);
  const colors = [];

  return sorted
    .reduce((result, [color]) => {
      const c = new Color(color);
      return result.findIndex((color) => new Color(color).deltaE2000(c) < 10) <
        0
        ? result.concat(color)
        : result;
    }, [])
    .slice(0, 9);
}

function SwatchColor({ color }: { color: string }) {
  const black = new Color("black");
  return (
    <div className={styles.Color}>
      <span
        style={{
          color: new Color(color).deltaE2000(black) > 50 ? "black" : "white",
          backgroundColor: color,
        }}
      >
        {color}
      </span>
    </div>
  );
}

export default function Palette({
  imgRef,
}: {
  imgRef: RefObject<HTMLImageElement>;
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [palette, setPalette] = useState<string[]>(() => []);

  useEffect(() => {
    if (imgRef.current) {
      const image = Object.assign(imgRef.current, {
        crossOrigin: "Anonymous",
      });
      // https://dev.to/producthackers/creating-a-color-palette-with-javascript-44ip
      const loadHandler = () => {
        if (canvasRef.current) {
          const canvas = canvasRef.current;
          canvas.width = image.width;
          canvas.height = image.height;
          const ctx = canvas.getContext("2d");
          if (ctx) {
            ctx.drawImage(image, 0, 0);
            setPalette(
              getPalette(ctx.getImageData(0, 0, canvas.width, canvas.height))
            );
          }
        }
      };
      image.addEventListener("load", loadHandler);
      return () => {
        image.removeEventListener("load", loadHandler);
      };
    }
  }, [canvasRef, imgRef]);

  return (
    <div className={styles.Palette}>
      <canvas ref={canvasRef} />
      <div className={styles.Swatch}>
        {palette.map((color, key) => (
          <SwatchColor key={key} color={color} />
        ))}
      </div>
    </div>
  );
}
